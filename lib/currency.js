...
CurrencyController.prototype.index = function(req, res) {
  ...
  if (self.bitstampRate === 0 || currentTime >= (self.timestamp + self.currencyDelay)) {
    ...
    request('https://api.coinpaprika.com/v1/coins/vrsc-verus-coin/markets', function(err, response, body) {
      if (err) {
        self.node.log.error(err);
      }
      if (!err && response.statusCode === 200) {

        self.bitstampRate = parseFloat(JSON.parse(body)[0].quotes.USD.price);
      }
